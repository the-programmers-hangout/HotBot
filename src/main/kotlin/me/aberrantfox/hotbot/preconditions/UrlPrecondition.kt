...
@Precondition
fun produceUrlPrecondition(config: Configuration, permissionManager: PermissionManager) = exit@{ event: CommandEvent ->
    ...
    if (permissionManager.canPerformAction(event.author, config.permissionedActions.sendURL)) return@exit Pass
