...
@CommandSet("Notes")
fun createCommandSet(databaseService: DatabaseService) = commands {
    command("note") {
        ...
        execute {
            ...
            val noteId = databaseService.notes.insertNote(target.id, it.author.id, note)
    ...
    command("removenote") {
        ...
        execute {
            ...
            val notesRemoved = databaseService.notes.removeNoteById(noteId)
    ...
    command("cleansenotes") {
        ...
        execute {
            val target = it.args.component1() as User
            val amount = databaseService.notes.removeAllNotesByUser(target.id)
    ...
    command("editnote") {
        ...
        execute {
            ...
            val noteId = it.args.component1() as Int
            val note = it.args.component2() as String
            databaseService.notes.replaceNote(noteId, note, it.author.id)
